...
export function ListingDetailPage() {
  ...
  return <main className="mx-auto max-w-[1360px] px-3 py-6 sm:px-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <div><img src={listing.image} alt={listing.title} className="aspect-video w-full rounded-2xl object-cover"/><div className="mt-5 flex items-start justify-between gap-4"><div><h1 className="text-xl font-black leading-8">{listing.title}</h1><p className="mt-1 text-2xl font-black text-[#e15f39]">{formatPrice(listing.price)} <span className="text-xs">دج</span></p></div><button onClick={() => toggleFavorite(listing.id)} className={`rounded-xl border p-3 ${favorite?'text-rose-500':'text-slate-500'}`}><Heart fill={favorite?'currentColor':'none'}/></button></div>
