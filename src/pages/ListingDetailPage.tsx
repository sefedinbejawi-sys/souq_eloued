...
import { ArrowUpDown, ChevronLeft, Copy, Heart, MapPin, MessageCircle, Phone, Share2, ShieldCheck } from 'lucide-react';
...
export function ListingDetailPage() {
  ...
  useEffect(() => { if (!listing) return; const description = `${listing.title} في ${listing.municipality} بسعر ${formatPrice(listing.price)} دج على سوق الوادي.`; document.title = `${listing.title} | سوق الوادي`; let meta = document.querySelector('meta[name="description"]'); if (!meta) { meta = document.createElement('meta'); meta.setAttribute('name', 'description'); document.head.appendChild(meta); } meta.setAttribute('content', description); let canonical = document.querySelector('link[rel="canonical"]'); if (!canonical) { canonical = document.createElement('link'); canonical.setAttribute('rel', 'canonical'); document.head.appendChild(canonical); } canonical.setAttribute('href', window.location.href); return () => { document.title = 'سوق الوادي — السوق المحلي في ولاية الوادي'; }; }, [listing]);
  useEffect(() => {
    let alive = true;
    const load = async () => {
  ...
  const shareListing = async () => { const payload = { title: listing.title, text: `${listing.title} - ${formatPrice(listing.price)} دج`, url: listingUrl }; if (navigator.share) { try { await navigator.share(payload); } catch { /* user cancelled */ } } else { await navigator.clipboard?.writeText(listingUrl); notify('تم نسخ رابط الإعلان.'); } };
  const copyLink = async () => { await navigator.clipboard?.writeText(listingUrl); notify('تم نسخ رابط الإعلان.'); };
