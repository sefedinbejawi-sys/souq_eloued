...
type Props = { active: string; counts?: Record<string, number>; onSelect: (name: string) => void; onShowAll: () => void };

export function CategoryGrid({ active, counts = {}, onSelect, onShowAll }: Props) {
  return <section id="categories" className="mt-8 scroll-mt-24">
    ...
    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 lg:grid-cols-8">
      {categories.map(c => <button key={c.name} onClick={() => onSelect(c.name)} className={`rounded-2xl border p-3 text-right transition hover:-translate-y-0.5 ${active === c.name ? 'border-[#f0ad96] bg-[#fff3ef]' : 'border-slate-200 bg-white hover:border-[#f0ad96]'}`}>
        ...
        <span className="mt-1 block text-[10px] leading-4 text-slate-400">{c.description}</span><span className="mt-2 block text-[10px] font-black text-[#e7663c]">{counts[c.name] || 0} إعلان</span>
      </button>)}
    </div>
  </section>;
}
