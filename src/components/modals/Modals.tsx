...
export function Modals() {
  ...
  const submitAuth = async () => {
    ...
    if (mode === 'signup' && !result.data.session) { setAuthMessage('تم إنشاء الحساب. افتح رسالة التأكيد في بريدك الإلكتروني ثم ارجع لتسجيل الدخول.'); return; }
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/55 p-3 backdrop-blur-sm" onMouseDown={() => setModal(null)}><div onMouseDown={e => e.stopPropagation()} className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-3xl bg-white p-5 shadow-2xl sm:p-7"><div className="flex items-center justify-between"><h3 className="text-lg font-black">{titles[modal]}</h3><button onClick={() => setModal(null)} className="rounded-xl bg-slate-100 p-2 text-slate-500"><X size={18}/></button></div>
    {modal === 'login' && <div className="mt-6"><div className="rounded-2xl bg-[#fff3ef] p-4 text-sm font-bold text-slate-700">{mode === 'login' ? 'سجل دخولك لإدارة إعلاناتك والمفضلة.' : 'أنشئ حساباً مجانياً وابدأ استقبال المشترين.'}</div>{authMessage && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold leading-6 text-amber-900">{authMessage}</div>}{mode === 'signup' && <input value={fullName} onChange={e => setFullName(e.target.value)} className="mt-4 h-12 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none" placeholder="الاسم الكامل"/>}<input type="email" value={email} onChange={e => setEmail(e.target.value)} className="mt-3 h-12 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none" placeholder="البريد الإلكتروني"/><input type="password" value={password} onChange={e => setPassword(e.target.value)} className="mt-3 h-12 w-full rounded-xl border border-slate-200 px-4 text-sm outline-none" placeholder="كلمة المرور (6 أحرف على الأقل)"/><button disabled={saving} onClick={submitAuth} className="mt-3 h-12 w-full rounded-xl bg-[#e7663c] text-sm font-black text-white disabled:opacity-60">{saving ? 'جارٍ التنفيذ...' : mode === 'login' ? 'تسجيل الدخول' : 'إنشاء الحساب'}</button><button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setAuthMessage(''); }} className="mt-3 w-full text-xs font-bold text-[#e7663c]">{mode === 'login' ? 'ليس لديك حساب؟ أنشئ حساباً' : 'لديك حساب؟ سجل الدخول'}</button></div>}
