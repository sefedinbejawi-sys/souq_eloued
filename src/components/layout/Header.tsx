...
export function Header() {
  ...
  const openAccount = () => navigate(session ? '/account' : '/');
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1360px] items-center gap-2 px-3 sm:h-[72px] sm:px-6">
        ...
        <div className="mr-auto flex items-center gap-1">
          ...
          <button onClick={session ? openAccount : () => setModal('login')} className="hidden items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50 md:flex"><LogIn size={17}/> {session ? 'حسابي' : 'دخول'}</button>
          <button onClick={openSell} className="flex h-10 items-center gap-1.5 rounded-xl bg-[#e7663c] px-3 text-xs font-black text-white shadow-sm hover:bg-[#cf5732] sm:px-4 sm:text-sm"><Plus size={17}/> أضف إعلاناً</button>
        </div>
      </div>
      {menu && <div className="border-t border-slate-100 bg-white p-4 lg:hidden">
        <div className="grid gap-2 text-sm font-bold text-slate-600">
          ...
          <button className="rounded-lg p-3 text-right hover:bg-slate-50" onClick={() => { if (session) navigate('/account'); else setModal('login'); setMenu(false); }}>حسابي</button>
        </div>
      </div>}
    </header>
  </>;
