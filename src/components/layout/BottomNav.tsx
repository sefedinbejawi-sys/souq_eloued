...
export function BottomNav() {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-40 grid h-[62px] grid-cols-4 border-t border-slate-200 bg-white/95 shadow-[0_-5px_20px_rgba(20,40,55,.08)] backdrop-blur md:hidden">
    ...
    <button onClick={() => session ? navigate('/account') : setModal('login')} className="flex flex-col items-center justify-center gap-0.5 text-slate-500"><UserRound size={19}/><span className="text-[10px] font-bold">حسابي</span></button>
  </nav>;
}
